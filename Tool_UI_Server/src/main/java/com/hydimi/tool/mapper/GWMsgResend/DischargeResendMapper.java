 ...
 */
@Mapper
public interface DischargeResendMapper {

    @Select("select top 2000 * from SPM_SPMPatLeftRpt(nolock) where IDCode=#{input.IDCode} and SyncStatus=#{input.syncStatus} and DelStatus='DelLogo001' and State='1'")
    List<SPMDischargeDTO> dischargeInfoQueryByIdcode(@Param("input") InfoQueryInput input, String provinceCode);

    @Select("<script>" +
            "select top 2000 * from SPM_SPMPatLeftRpt(nolock) " +
            ...
            "</script>")
    List<SPMDischargeDTO> dischargeInfoQuery(@Param("input") InfoQueryInput input, String provinceCode);

    @Select("select distinct top 2000 SyncError from SPM_SPMPatLeftRpt(nolock) " +
            "where DelStatus='DelLogo001' and State='1' and SyncStatus=#{input.syncStatus} and ZoneCd like '${provinceCode}%' " +
    ...
    @Select("select top 2000 * from SPM_SPMPatLeftRpt(nolock) " +
    ...
    List<SPMDischargeDTO> dischargeNotSyncQuery(@Param("input") InfoQueryInput input, String provinceCode);
}

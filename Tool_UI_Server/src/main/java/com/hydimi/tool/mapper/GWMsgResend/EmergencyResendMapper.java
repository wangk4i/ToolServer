 ...
 */
@Mapper
public interface EmergencyResendMapper {

    @Select("select top 2000 * from SPM_EmrDeal(nolock) where IDCode=#{input.IDCode} and SyncStatus=#{input.syncStatus} and DelStatus='DelLogo001' and State='1'")
    List<SPMEmergencyDTO> emergencyInfoQueryByIdcode(@Param("input") InfoQueryInput input, String provinceCode);

    @Select("<script>" +
            "select top 2000 * from SPM_EmrDeal(nolock) " +
            ...
            "</script>")
    List<SPMEmergencyDTO> emergencyInfoQuery(@Param("input") InfoQueryInput input, String provinceCode);

    @Select("select distinct top 2000 SyncError from SPM_EmrDeal(nolock) " +
            "where DelStatus='DelLogo001' and State='1' and SyncStatus=#{input.syncStatus} and ZoneCd like '${provinceCode}%' " +
    ...
    @Select("select top 2000 * from SPM_EmrDeal(nolock) " +
    ...
    List<SPMEmergencyDTO> emergencyNotSyncQuery(@Param("input") InfoQueryInput input, String provinceCode);
}

layui.define(['jquery'], function (exports) {
    ...
    var CityPicker = function (element, options) {
        ...
        this.$class = element.substring(1,element.length);
        this.$dropdown = null;
    ...
    CityPicker.prototype = {
        ...
        display:function(){
            ...
            var str = ["provinceId","cityId","districtId","townId","villageId"];
            var index = -1;
            ...
            for(var i=0;i<index;i++){
                var div_class=str[i];
                console.log("."+this.$class+"."+div_class)
                $("."+this.$class+" ."+div_class).wrapAll($("<div id='shade' style='pointer-events: none;font-size:14px'></div>").css({
        ...
        render: function () {
            ...
            var p = this.getPosition(),
                ...
                textspan = '<span class="city-picker-span" style="' +
                    ...
                    (placeholder ? '<span class="placeholder" id="citypickerspan1">' + placeholder + '</span>' : '') +
                    '<span class="title" style="width:187px" id="citypickerspan2"></span><div class="arrow"></div>' + '<i class="icon_ca"></i></span>',
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;width:353px;display:block;z-index:99999999999999999999;' + '">' +
                    '<div class="city-select-wrap">' +
                    '<div class="city-select-tab">' +
                    '<a class="active" data-count="' + this.PROVINCE + '">省份</a>' +
                    (this.includeDem(this.CITY) ? '<a data-count="' + this.CITY + '">城市</a>' : '') +
                    (this.includeDem(this.DISTRICT) ? '<a data-count="' + this.DISTRICT + '">区县</a>' : '') + 
                    (this.includeDem(this.TOWN) ? '<a data-count="' + this.TOWN + '">乡镇</a>' : '') +  
                    (this.includeDem(this.VILLAGE) ? '<a data-count="' + this.VILLAGE + '">村</a>' : '') + '</div>' + 
                    '<div class="city-select-content ' +this.$class+'">' +
                    ...
                    (this.includeDem(this.TOWN) ? '<div class="city-select ' + this.TOWN + '" data-count="' + this.TOWN + '"></div>' : '') +
                    (this.includeDem(this.VILLAGE) ? '<div class="city-select ' + this.VILLAGE + '" data-count="' + this.VILLAGE + '"></div>' : '') +
                    '</div></div>';
        ...
        defineDems: function () {
            ...
            if (this.options.townname !== "") {
                this.TOWN = this.options.townname;
            }
            if (this.options.villagename !== "") {
                this.VILLAGE = this.options.villagename;
            }
            $.each([this.PROVINCE, this.CITY, this.DISTRICT,this.TOWN,this.VILLAGE], $.proxy(function (i, type) {
                if (!stop) {
                    this.dems.push(type);
                }
        ...
        bind: function () {
            ...
            if (this['$' + this.PROVINCE]) {
                this['$' + this.PROVINCE].on(EVENT_CHANGE, (this._changeProvince = $.proxy(function () {
                    ...
                    this.output(this.TOWN);
                    this.output(this.VILLAGE);
                    this.tab(this.CITY);
                }, this)));
            }
            ...
            if (this['$' + this.DISTRICT]) {
                this['$' + this.DISTRICT].on(EVENT_CHANGE, (this._changeDistrict = $.proxy(function () {
                    this.output(this.TOWN);
                    this.output(this.VILLAGE);
                    this.tab(this.TOWN);
                }, this)));
            ...
            if (this['$' + this.TOWN]) {
                this['$' + this.TOWN].on(EVENT_CHANGE, (this._changeTown = $.proxy(function () {
                    this.output(this.VILLAGE);
                    this.tab(this.VILLAGE);
                }, this)));
            }
        },
            ...
            code = (
                type === PROVINCE ? 86 :
                    type === CITY ? this['$' + PROVINCE] && this['$' + PROVINCE].find('.active').data('code') :
                        ...
                        type === TOWN ? this['$' + DISTRICT] && this['$' + DISTRICT].find('.active').data('code') :
                        type === VILLAGE ? this['$' + TOWN] && this['$' + TOWN].find('.active').data('code') : code
            );
    ...
    CityPicker.DEFAULTS = {
        ...
        townname: '',//input hidden 的值
        villagename:'',//input hidden 的值
        province: '四川省',// 默认省份名称
        city: '',// 默认地市名称
        district: '',// 默认区县名称
        town: '',// 默认乡镇名称

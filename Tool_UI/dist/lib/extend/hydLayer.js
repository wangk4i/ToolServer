...
layui.define(["layer"], function (exports) {
    var hydLayer = {
        ...
        confirm: function (content, yesFunction) {
            var options = {skin: hydLayer.skinChoose(), icon: 3, title: "提示", anim: hydLayer.animChoose()};
        ...
        open: function (title, content, width, height, successFunction, endFunction) {
            layer.open({
                ...
                maxmin: true,
                shade: 0.5,
                anim: hydLayer.animChoose(),
                area: [width, height],
                content: content,
                zIndex: layer.zIndex,
                skin: hydLayer.skinChoose(),
        ...
        msg: {
            ...
            greenTick: function (content, callbackFunction) {
                var options = {icon: 1, time: hydLayer.msg.time, anim: hydLayer.animChoose()};
                layer.msg(content, options, callbackFunction);
            },
            // 红色叉
            redCross: function (content, callbackFunction) {
                var options = {icon: 2, time: hydLayer.msg.time, anim: hydLayer.animChoose()};
                layer.msg(content, options, callbackFunction);
            },
            // 黄色问号
            yellowQuestion: function (content, callbackFunction) {
                var options = {icon: 3, time: hydLayer.msg.time, anim: hydLayer.animChoose()};
                layer.msg(content, options, callbackFunction);
            },
            // 灰色锁
            grayLock: function (content, callbackFunction) {
                var options = {icon: 4, time: hydLayer.msg.time, anim: hydLayer.animChoose()};
                layer.msg(content, options, callbackFunction);
            },
            // 红色哭脸
            redCry: function (content, callbackFunction) {
                var options = {icon: 5, time: hydLayer.msg.time, anim: hydLayer.animChoose()};
                layer.msg(content, options, callbackFunction);
            },
            // 绿色笑脸
            greenLaugh: function (content, callbackFunction) {
                var options = {icon: 6, time: hydLayer.msg.time, anim: hydLayer.animChoose()};
                layer.msg(content, options, callbackFunction);
            },
            // 黄色感叹号
            yellowSigh: function (content, callbackFunction) {
                var options = {icon: 7, time: hydLayer.msg.time, anim: hydLayer.animChoose()};
                layer.msg(content, options, callbackFunction);
            }
        },
        /**
        ...
        successMsg: function (content) {
            layer.msg(content, {icon: 1, time: 1000});
        },
        /**
         * 在页面操作成功后弹窗提示
         * @param content

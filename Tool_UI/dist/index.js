...
layui.extend({
    ...
    ,renderPage = function(){
        ...
        */
        ...
        view().render(path.join('/')).then(function(res,title){
             ...
             * 获取local中的tab页签切换下标
             */
            const tabsindex = layui.data("mhdrStorage").tabsindex;
            ...
            let BADGE = '';
            BADGE = '<span class="layui-badge-dot  layui-bg-green" style="margin-right:10px;"></span><span style="color:green;">'
            ...
            if(setter.pageTabs && pathURL !== '/'){
                if(!matchTo){
                    ...
                    element.tabAdd(FILTER_TAB_TBAS, {
                        title: BADGE + (res.title || '新标签页') +'</span>'
                        ,id: pathURL
                        ,attr: router.href
                    });
                }
    ...
    //加载外部Css
    layui.link(
        setter.base + 'style/health.css?v='+ layui.cache.version
    );
    ...
    //扩展 资源 模块
    layui.each(setter.resConfigs, function(index, item){
        var mods = {};
